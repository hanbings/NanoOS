...
PUBLIC int do_open()
{
	...
	if ((fd < 0) || (fd >= NR_FILES))
		panic("[ERRO][FS]filp[] is full (PID:%d)", proc2pid(pcaller));
	...
	for (i = 0; i < NR_FILE_DESC; i++)
		if (f_desc_table[i].fd_inode == 0)
			break;
	if (i >= NR_FILE_DESC)
		panic("[ERRO][FS]f_desc_table[] is full (PID:%d)", proc2pid(pcaller));
	...
	if (flags & O_CREAT) {
		if (inode_nr) {
			printl("[ERRO][FS] file not exists\n");
			return -1;
		}
		else {
...
PRIVATE int alloc_imap_bit(int dev)
{
	...
	/* no free bit in imap */
	panic("[ERRO][FS]inode-map is probably full.\n");
