...
PUBLIC void task_fs()
{
	printl("[INFO][FS] Task FS Running.\n");
	...
	while (1) {
		...
		case STAT:
			...
			break;
		default:
			dump_msg("[WARN][FS]FS::unknown message:", &fs_msg);
			assert(0);
			break;
		}
		...
		switch (msgtype) {
		case UNLINK:
			dump_fd_graph("%s[INFO][FS]Just Finished. (pid:%d)",
...
PRIVATE void mkfs()
{
	...
	printl("[INFO][FS] dev size: 0x%x sectors\n", geo.size);
	...
	printl("[INFO][FS] devbase:0x%x00, sb:0x%x00, imap:0x%x00, smap:0x%x00\n"
...
PRIVATE void read_super_block(int dev)
{
	...
	for (i = 0; i < NR_SUPER_BLOCK; i++)
		if (super_block[i].sb_dev == NO_DEV)
			break;
	if (i == NR_SUPER_BLOCK)
		panic("[WARN][FS]super_block slots used up");
...
PUBLIC struct super_block * get_super_block(int dev)
{
	...
	panic("[ERRO][FS]super block of devie %d not found.\n", dev);
...
PUBLIC struct inode * get_inode(int dev, int num)
{
	...
	if (!q)
		panic("[ERRO]the inode table is full");
