...
PUBLIC int do_unlink()
{
	...
	if (strcmp(pathname , "/") == 0) {
		printl("[INFO][FS] FS:do_unlink():: cannot unlink the root\n");
		return -1;
	}

	int inode_nr = search_file(pathname);
	if (inode_nr == INVALID_INODE) {	/* file not found */
		printl("[INFO][FS] FS::do_unlink():: search_file() returns "
			"invalid inode: %s\n", pathname);
		return -1;
	}
	...
	printl("[INFO][FS] cannot remove file %s, because "
	...
	if (pin->i_cnt > 1) {	/* the file was opened */
			printl("[INFO][FS] cannot remove file %s, because pin->i_cnt is %d.\n",
		       pathname, pin->i_cnt);
		return -1;
	}
