...
PUBLIC void task_hd()
{
	...
	while (1) {
		...
		default:
			dump_msg("[ERRO]HD driver::unknown msg", &msg);
			spin("[ERRO]FS::main_loop (invalid msg.type)");
			break;
		}
	...
	while (bytes_left) {
		...
		if (p->type == DEV_READ) {
			...
			if (!waitfor(STATUS_DRQ, STATUS_DRQ, HD_TIMEOUT))
				panic("[ERRO][HD]HardDisk writing error.");
...
PRIVATE void print_hdinfo(struct hd_info * hdi)
{
	...
	for (i = 0; i < NR_PART_PER_DRIVE + 1; i++) {
		printl("[INFO][HD] %sPART_%d: base %d(0x%x), size %d(0x%x) (in sector)\n",
		       i == 0 ? " " : "     ",
		       i,
		       hdi->primary[i].base,
		       hdi->primary[i].base,
...
PRIVATE void print_identify_info(u16* hdinfo)
{
	...
	printl("[INFO][HD] LBA supported: %s\n",
	       (capabilities & 0x0200) ? "Yes" : "No");

	int cmd_set_supported = hdinfo[83];
	printl("[INFO][HD] LBA48 supported: %s\n",
	       (cmd_set_supported & 0x0400) ? "Yes" : "No");

	int sectors = ((int)hdinfo[61] << 16) + hdinfo[60];
	printl("[INFO][HD] HD size: %dMB\n", sectors * 512 / 1000000);
}
