...
PUBLIC void exception_handler(int vec_no, int err_code, int eip, int cs, int eflags)
{
	...
	disp_color_str("[ERRO][Core]Exception! --> ", text_color);
	disp_color_str(err_description[vec_no], text_color);
	disp_color_str("\n\n", text_color);
	disp_color_str("[ERRO][Core]EFLAGS:", text_color);
	disp_int(eflags);
	disp_color_str("CS:", text_color);
	disp_int(cs);
	disp_color_str("[ERRO][Core]EIP:", text_color);
	disp_int(eip);

	if(err_code != 0xFFFFFFFF){
		disp_color_str("[ERRO][Core]Error code:", text_color);
		disp_int(err_code);
	}
}
