...
PUBLIC void task_mm()
{
	...
	while (1) {
		...
		case WAIT:
			...
			break;
		default:
			dump_msg("[INFO][MM] MM::unknown msg", &mm_msg);
			assert(0);
			break;
		}
...
PUBLIC int alloc_mem(int pid, int memsize)
{
	...
	if (memsize > PROC_IMAGE_SIZE_DEFAULT) {
		panic("[ERRO][MM] unsupported memory request: %d. "
	...
	if (base + memsize >= memory_size)
		panic("[ERRO][MM]memory allocation failed. pid:%d", pid);
